...
from target_postgres import json_schema, main, postgres, singer_constants as singer, singer_stream
    ...
    for prop, value in old_obj.items():
        ...
        if isinstance(value, dict):
            ...
            for item in value:
                ...
                new_subobj[singer_stream.singer_constants.LEVEL_FMT.format(level)] = row_index
                new_subpks[singer_stream.singer_constants.LEVEL_FMT.format(level)] = row_index
...
def assert_records(conn, records, table_name, pks, match_pks=False):
    ...
    with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
        ...
        for record in records:
            ...
            subpks = {}
            for pk in pks:
                subpks[singer_stream.singer_constants.SOURCE_PK_PREFIX + pk] = persisted_record[pk]
            assert_record(record, persisted_record, subtables, subpks)

        if match_pks:
            assert sorted(list(persisted_records.keys())) == sorted(records_pks)

        sub_pks = list(map(lambda pk: singer_stream.singer_constants.SOURCE_PK_PREFIX + pk, pks))
